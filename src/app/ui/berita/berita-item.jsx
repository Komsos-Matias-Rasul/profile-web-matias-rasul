    ...
    <Link href={`/berita/${slugify(item.title)}`} className="flex w-full">
      ...
      >
        ...
        <CardFooter className="flex-col items-start text-left p-2 gap-2 md:p-3 md:gap-3 lg:p-3 lg:gap-1">
          <b className="text-sm md:text-md lg:text-lg text-blue-secondary">
            {item.bidang}
          </b>
          <b className="text-md md:text-lg lg:text-xl font-bold text-blue-primary hover:underline">
            {item.title}
          </b>
          <p className="text-sm lg:text-md my-2 ">
            {item?.isiBerita.substring(0, 85)}...
          </p>
